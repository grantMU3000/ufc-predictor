...
def _remap_ids(
    series: pd.Series, pandas_id_to_url: dict, url_to_db_id: dict
) -> pd.Series:
...
def load_bouts(
    ...
    fighter_pandas_id_to_url: dict,
    fighter_url_to_db_id: dict,
    event_pandas_id_to_url: dict,
    event_url_to_db_id: dict,
) -> dict[str, int]:
    """
    ...
    """
    df = bouts_df.copy()
    df["fighter_red_id"] = _remap_ids(
        df["fighter_red_id"], fighter_pandas_id_to_url, fighter_url_to_db_id
    )
    df["fighter_blue_id"] = _remap_ids(
        df["fighter_blue_id"], fighter_pandas_id_to_url, fighter_url_to_db_id
    )
    df["winner_id"] = _remap_ids(
        df["winner_id"], fighter_pandas_id_to_url, fighter_url_to_db_id
    )
    df["event_id"] = _remap_ids(
        df["event_id"], event_pandas_id_to_url, event_url_to_db_id
    )
...
def load_bout_stats(
    engine,
    bout_stats_df: pd.DataFrame,
    bout_pandas_id_to_url: dict,
    bout_url_to_db_id: dict,
    fighter_pandas_id_to_url: dict,
    fighter_url_to_db_id: dict,
) -> int:
    df = bout_stats_df.copy()
    df["bout_id"] = _remap_ids(df["bout_id"], bout_pandas_id_to_url, bout_url_to_db_id)
    df["fighter_id"] = _remap_ids(
        df["fighter_id"], fighter_pandas_id_to_url, fighter_url_to_db_id
    )
    ...
    if len(df) < before:
        print(
            f"load_bout_stats: dropped {before - len(df)} rows — "
...
def load_all(fighters_df, events_df, bouts_df, bout_stats_df) -> None:
    ...
    fighter_pandas_id_to_url = dict(
        zip(fighters_df["fighter_id"], fighters_df["source_url"])
    )
    ...
    bout_url_to_db_id = load_bouts(
        engine,
        bouts_df,
        fighter_pandas_id_to_url,
        fighter_url_to_db_id,
        event_pandas_id_to_url,
        event_url_to_db_id,
    )
